...
def changeXY(str):
  ...
  if(len(str) == 0):
    ...
    new += "y" + changeXY(str[1:])
  else:
    new += str[0] + changeXY(str[1:])
  return new 
...
def changePi(str):
  ...
  if(len(str) == 0):
    ...
    new += "3.14" + changePi(str[2:])
  else:
    new += str[0] + changePi(str[1:])
  return new
...
def countPairs(str):
  if(len(str) == 1 or len(str) ==0) or len(str) ==2:
    return 0
  elif(str[0] == str[2]):
...
def nestParen(str):
  if(str == ""):
    ...
    return False
  elif(len(str) <3 and str[0] == "(" and str[-1] == ")"):
    return True
...
def strCount(str, sub):
  if(str != sub and len(str) == len(sub)):
    return 0
  elif( str == sub):
    return 1
...
def strDist(str, sub):
  if(len(str) < len(sub)):
    ...
    return len(str)
  elif(str.startswith(sub) == False):
    return strDist(str[1:], sub)
  elif(str.endswith(sub) == False):
