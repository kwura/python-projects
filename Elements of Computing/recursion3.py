...
def groupSum5(start, nums, target):
  if( start >= len(nums)):
    ...
    if( nums[start] % 5 == 0 and start < len(nums) - 1):
      if(nums[start+1] !=1):
...
def splitArrayHelper(index, nums, first, second):
  if( index >= len(nums)):
    if(first == second):
      return True
    else:
    ...
    return splitArrayHelper(index+1, nums, first + nums[index], second) or splitArrayHelper(index+1, nums, first, second+ nums[index])
...
def splitOdd10Helper(index, nums, ten, odd):
  if(index >= len(nums)):
    if(ten % 10 == 0 and odd % 2 != 0):
      return True
    else:
      return False
  else:
    return splitOdd10Helper(index+1, nums, ten+nums[index], odd) or splitOdd10Helper(index+1, nums, ten, odd +nums[index])
...
def split53Helper(index, nums, fives, threes):
  if(index >= len(nums)):
    if( fives == threes):
      return True
    else:
    ...
    if(nums[index] % 5 == 0):
      return split53Helper(index + 1, nums, fives + nums[index], threes)
    elif(nums[index] % 3 == 0 and nums[index] % 5 != 0):
      return split53Helper(index + 1, nums, fives, threes + nums[index])
    else:
      return split53Helper(index + 1, nums, fives + nums[index], threes) or split53Helper(index +1, nums, fives, threes + nums[index])
